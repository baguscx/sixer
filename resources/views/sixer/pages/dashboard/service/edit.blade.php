                        ...
                        <form action="{{ route('member.service.update', [$service->id]) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="">
                                <div class="px-4 py-5 sm:p-6">
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            ...
                                            @error('title')
                                                <p class="text-red-500 mb-3 text-sm">{{ $messages }}</p>
                                            @enderror
                                        </div>
                                        <div class="col-span-6">
                                        ...
                                        <div class="col-span-6">
                                            ...
                                            <div class="grid grid-cols lg:grid-cols-3 md:grid-cols-2 gap-4">
                                                @forelse ($thumbnail_service as $thumbnail_item)
                                                    <div class="mb-2">
                                                        <img src="{{ url(Storage::url($thumbnail_item->thumbnail)) }}" alt="" class="inline object-cover w-20 h-20 rounded" for="choose">

                                                        <input type="file" placeholder="Thumbnail" name="{{ 'thumbnails['.$thumbnail_item->id.']' }}" id="thumbnails" autocomplete="thumbnails" class="block w-full py-3 pl-5 mt-3 border-gray-300 rounded shadow-sm focus:ring-green-500 sm:text-sm">
                                                    </div>
                                                @empty
                                        ...
                                        <div class="col-span-6">
                                            ...
                                            @forelse ($advantage_user as $advantage_user_item)
                                                <input placeholder="Keunggulan 1" type="text" name="{{ ('advantage-users['.$advantage_user_item->id.']') }}" id="advantage-users" autocomplete="advantage-users" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" value="{{ $advantage_user_item->advantage ?? '' }}" required>
                                            @empty
                                ...
                                <div class="px-4 py-3 text-right sm:px-6">
                                    <a href="{{ route('member.service.index') }}" class="inline-flex justify-center px-4 py-2 mr-4 text-sm font-medium text-gray-700 bg-white border border-gray-600 rounded-lg shadow-sm hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300" onclick="return confirm('Are you sure to cancel?')">
                                        Cancel
                                    </a>
                                    <button type="submit" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" onclick="return confirm('are you sure to save changes?')">
                                        Save Changes
                                    </button>
                                </div>
                            </div>
...
@push('after-script')
    ...
    <script type="text/javascript">
        ...
        $("#addAdvantagesRow").click(function() {
            var html = '';
            html += '<input placeholder="Keunggulan" type="text" name="advantages_user[]" id="advantage-user" autocomplete="advantage-user" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" required>';
