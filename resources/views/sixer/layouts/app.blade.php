...
<head>
    @include('sixer.includes.dashboard.meta')
    <title>@yield('title') | SERV</title>
    @stack('before-style')
    @include('sixer.includes.dashboard.style')
    @stack('after-style')
</head>
<body class="antialiased">
    <div class="flex h-screen bg-serv-services-bg" :class="{'overflow-hidden': isSideMenuOpen}">
            @include('sixer.components.dashboard.desktop')
            ...
            @include('sixer.components.dashboard.mobile')
            <div class="flex flex-col flex-1 w-full">

            @include('sixer.components.dashboard.header')
            {{-- @include('sweetalert::alert') --}}
            @yield('content')
        </div>
    </div>
    @stack('before-script')
    @include('sixer.includes.dashboard.script')
    @stack('after-script')
</body>
</html>
